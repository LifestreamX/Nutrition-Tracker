...
export async function POST(request: Request) {
  try {
    ...
    // if (processedFoodLogs.length === 0) {
    //   // Handle the case where the foodLog array is empty
    //   // For example, you can delete existing food logs associated with the user
    //   const deletedFoodLogs = await prisma.foodLog.deleteMany({
    //     where: {
    //       userId: userEmail,
    //     },
    //   });

    //   return NextResponse.json(deletedFoodLogs);
    // }
